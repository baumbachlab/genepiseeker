...
void
CSVParser::
parse(const std::string & filename, char sep, char quote, bool trim) {
	num_rows_ = 0;
	num_columns_ = undefined_uint();
	fields_.clear();
	...
	while(std::getline(csv_file, row)) {
		parse_row_(row, row_id++, sep, quote, trim);
	}
	csv_file.close();
}
...
void
CSVParser::
parse_row_(const std::string & row, std::size_t row_id, char sep, char quote, bool trim) {
	std::size_t num_fields_in_row{misc::tokenize(row, sep, quote, trim, fields_, false)};
