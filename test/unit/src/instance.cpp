...
TEST_CASE("Quantitative Instance") {
	...
	for (std::size_t fold_id{0}; fold_id < 3; fold_id++) {
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::JSON_EPIGEN, "../../../data/EpiGEN/0_1_ASW.json", 3, fold_id, epi::options::DataPurpose::TRAINING));
		std::size_t num_training_inds{instance.num_inds()};
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::JSON_EPIGEN, "../../../data/EpiGEN/0_1_ASW.json", 3, fold_id, epi::options::DataPurpose::VALIDATION));
...
TEST_CASE("Categorical Instance") {
	...
	std::vector<std::size_t> accepted_fold_sizes{min_fold_size, min_fold_size + 1};
	for (std::size_t fold_id{0}; fold_id < 3; fold_id++) {
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::JSON_EPIGEN, "../../../data/EpiGEN/1_1_ASW.json", 3, fold_id, epi::options::DataPurpose::TRAINING));
		std::size_t num_training_inds{instance.num_inds()};
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::JSON_EPIGEN, "../../../data/EpiGEN/1_1_ASW.json", 3, fold_id, epi::options::DataPurpose::VALIDATION));
		std::size_t num_validation_inds{instance.num_inds()};
		sum_num_validation_inds += num_validation_inds;
		REQUIRE(num_training_inds + num_validation_inds == total_num_inds);
		REQUIRE_THAT(accepted_fold_sizes, Catch::VectorContains(num_validation_inds));
...
TEST_CASE("Categorical CSV Instances") {
	...
	std::vector<std::size_t> accepted_fold_sizes{min_fold_size, min_fold_size + 1};
	for (std::size_t fold_id{0}; fold_id < 3; fold_id++) {
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::CSV_SNPS_AS_COLUMNS_FIRST, "../../../data/MACOED/NME_models_snp100/00.1600.0.antesnp100.txt", 3, fold_id, epi::options::DataPurpose::TRAINING));
		std::size_t num_training_inds{instance.num_inds()};
		REQUIRE_NOTHROW(instance.load(epi::options::InputFormat::CSV_SNPS_AS_COLUMNS_FIRST, "../../../data/MACOED/NME_models_snp100/00.1600.0.antesnp100.txt", 3, fold_id, epi::options::DataPurpose::VALIDATION));
		std::size_t num_validation_inds{instance.num_inds()};
		sum_num_validation_inds += num_validation_inds;
		REQUIRE(num_training_inds + num_validation_inds == total_num_inds);
		REQUIRE_THAT(accepted_fold_sizes, Catch::VectorContains(num_validation_inds));
